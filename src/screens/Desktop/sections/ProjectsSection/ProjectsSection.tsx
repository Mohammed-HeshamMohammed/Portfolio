      ...
      <div className="inline-flex flex-col items-center gap-6 sm:gap-[39px] relative flex-[0_0_auto] w-full">
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 w-full max-w-[1299px] px-4">
          {serviceCards.map((card, index) => (
            <div
              key={index}
              className={`inline-flex flex-col items-center justify-center gap-4 sm:gap-6 relative flex-[0_0_auto] w-full h-[300px] sm:h-[400px] lg:h-[508px] bg-gradient-to-br ${card.gradient} rounded-2xl sm:rounded-3xl p-4 sm:p-6 lg:p-8`}
            >
              ...
              <Button
                className="flex w-[60px] sm:w-[80px] lg:w-[114px] h-[60px] sm:h-[80px] lg:h-[114px] items-center justify-center gap-2.5 p-[12px] sm:p-[16px] lg:p-[21px] absolute bottom-2 sm:bottom-3 lg:bottom-4 right-2 sm:right-3 lg:right-4 bg-white/20 rounded-[30px] sm:rounded-[40px] lg:rounded-[57px] hover:bg-white/30"
                variant="ghost"
              >
        ...
        >
          {paginationDots.map((dot, index) => (
            <div
              key={index}
              className={`relative h-[12px] sm:h-[15.08px] rounded-[15px] sm:rounded-[20.74px] transition-all duration-300 ${dot.width} ${dot.color}`}
            />
          ))}
        </nav>
      </div>
