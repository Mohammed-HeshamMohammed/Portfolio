...
export const ServicesSection = (): JSX.Element => {
  ...
  return (
    <section className="flex flex-col items-center relative w-full px-4 sm:px-8 lg:px-0">
      <nav className="flex w-full max-w-[1298px] h-auto sm:h-[86px] items-center justify-between px-2.5 py-2 sm:py-0 relative bg-neutral-900 rounded-[25px] sm:rounded-[50px] border-[none] backdrop-blur-[7.5px] backdrop-brightness-[100%] [-webkit-backdrop-filter:blur(7.5px)_brightness(100%)] before:content-[''] before:absolute before:inset-0 before:p-px before:rounded-[25px] sm:before:rounded-[50px] before:[background:linear-gradient(250deg,rgba(255,255,255,1)_0%,rgba(203,203,203,0)_100%)] before:[-webkit-mask:linear-gradient(#fff_0_0)_content-box,linear-gradient(#fff_0_0)] before:[-webkit-mask-composite:xor] before:[mask-composite:exclude] before:z-[1] before:pointer-events-none flex-wrap sm:flex-nowrap gap-2">
        {navigationItems.map((item, index) => (
          <Button
            key={item.label}
            variant="ghost"
            className={`inline-flex items-center justify-center gap-1 sm:gap-2.5 px-3 sm:px-6 lg:px-10 py-2 sm:py-3 lg:py-5 relative flex-[0_0_auto] rounded-[30px] sm:rounded-[60px] overflow-hidden h-auto text-sm sm:text-base lg:text-xl ${
        ...
        <div className="flex w-auto sm:w-[200px] lg:w-[307px] h-auto sm:h-[86px] items-center justify-center gap-2.5 px-3 sm:px-6 lg:px-10 py-2 sm:py-5 relative rounded-[30px] sm:rounded-[60px] overflow-hidden">
