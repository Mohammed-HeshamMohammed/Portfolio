    ...
    <section className="flex flex-col min-h-[600px] sm:h-[806px] items-center justify-center gap-12 sm:gap-24 px-4 sm:px-8 lg:px-[71px] py-[60px] sm:py-[122px] relative self-stretch w-full bg-[#f2f3f6] rounded-[25px] sm:rounded-[50px] overflow-hidden">
      <div className="flex w-full max-w-[1299px] min-h-[400px] sm:h-[600px] items-center justify-between relative flex-col lg:flex-row gap-8 lg:gap-0">
        ...
        <div className="flex flex-col items-start gap-6 sm:gap-8 lg:gap-[47px] relative flex-1 grow w-full lg:w-auto">
          ...
          <Button
            variant="outline"
            className="inline-flex items-center justify-center gap-2.5 px-8 sm:px-12 lg:px-[59px] py-4 sm:py-6 lg:py-[33px] relative flex-[0_0_auto] rounded-[20px] sm:rounded-[25px] lg:rounded-[32px] border border-solid border-[#f72585] bg-transparent hover:bg-[#f72585] hover:text-white transition-all duration-300 h-auto mx-auto lg:mx-0"
            onClick={() => {
              const element = document.querySelector('#contact');
              element?.scrollIntoView({ behavior: 'smooth' });
            }}
          >
            <span className="relative w-fit mt-[-1.00px] [font-family:'Lufga-SemiBold',Helvetica] font-semibold text-[#f72585] group-hover:text-white text-lg sm:text-2xl lg:text-[32px] tracking-[-0.48px] leading-[normal] transition-colors duration-300">
              Hire me
            </span>
          </Button>
        </div>
